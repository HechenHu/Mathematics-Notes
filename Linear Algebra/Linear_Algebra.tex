...
The subspace $ W $ as above is called the subspace \textit{generated} by $ \vec{v}_1,\vec{v}_2,\cdots,\vec{v}_n $. If $ W=V $, then we say that $ \vec{v}_1,\vec{v}_2,\cdots,\vec{v}_n $ \textit{generate} $ V $.
\begin{Definition}
	The \textit{dot product} or \textit{scalar product} of two vectors is defined as the sum of the product of their corresponding components. It has the following properties:
	\begin{enumerate}
		\item The commutativity of dot product.
		\item The distributivity of dot product over vector addition and vice versa.
		\item The associativity of scalar multiplication and dot product.
	\end{enumerate}
Two vectors are \textit{perpendicular} or \textit{orthogonal} if their dot product is zero.
\end{Definition}
...
\section{Bases}
	...
	\end{equation}
	has only the trivial solution. Otherwise it's said to be \textit{linearly dependent}.
\end{Definition}
\begin{Definition}
	A collection of linearly independent vectors that generated the vector space $ V $ is called a \textit{basis} of $ V $.
\end{Definition}
\begin{Definition}
	Let $ V $ be a vector space and $ \mathscr{B} $ be a basis for it. The \textit{coordinate with respect to basis $ \mathscr{B} $} of an element is a n-tuple of numbers such that the desired element can be expressed with the linear combination of the n-tuple and basis elements. By the definition of basis, the coordinate of any element is unique.
\end{Definition}	
\begin{Definition}
	The \textit{maximal subset of linearly independent elements} is a subset of any collection of vectors of a vector space that adding another element from the original collection that is not in this maximal subset will result in the loss of linearly independence.
\end{Definition}
\begin{Theorem}
	The maximal subset of linearly independent elements of any collection of vectors that generate the vector space is a basis for it.
\end{Theorem}
\section{Dimension of a Vector Space}	
\begin{Theorem}
	Let $ V $ be a vector space over the field $ K $. Let $ \mathscr{B} $ be a basis that has $ m $ elements. Then any collection of more than $ m $ vectors in $ V $ are linearly dependent.
\end{Theorem}	
\begin{proof}
	Let the basis $ \mathscr{B} $ be $ \{\vec{v}_1,\cdots,\vec{v}_m\} $ and the collection of $ n $ vectors($ n>m $) in $ V $ be $ \vec{w}_1,\cdots,\vec{w}_n $. Assume this collection of vectors are linearly independent. Since $ \mathscr{B} $ is a basis, the equation
	\begin{equation}
		\vec{w}_1 = a_1 \vec{v}_1 + \cdots + a_m\vec{v_m} \nonumber
	\end{equation}
	holds for some elements in the field $ K $ that are not all zero. Without loss of generality, we can assume that $ a_1\neq 0 $(otherwise we could just rearrange the equation to make it so). Solve for $ \vec{v}_1 $
	\begin{equation}
		\vec{v}_1 = a_1^{-1}\vec{w}_1 -a_1^{-1}a_2 \vec{v}_2 -\cdots a_1^{-1}a_m \vec{v}_m \nonumber
	\end{equation}
	Then the subspace generated by $ \vec{w}_1,\vec{v}_2,\cdots,\vec{v}_m $ is all of $ V $ since it contains $ \vec{v}_1 $. Now assume by induction that there is an integer $ r $, $ 1 \leqslant r < m $, such that after a suitable renumbering of $ \vec{v}_1,\cdots,\vec{v}_m $, the elements $ \vec{w}_1,\cdots,\vec{w}_r,\vec{v}_{r+1},\cdots,\vec{v}_m $ generate $ V $. Then the equation
	\begin{equation}
		\vec{w}_{r+1}=b_1 \vec{w}_1 + \cdots + b_r \vec{w}_r + c_{r+1}\vec{v}_{r+1}+\cdots + c_m \vec{v}_m \nonumber
	\end{equation}
	holds for some element in $ K $. $ c_{r+1},\cdots, c_m$ can't be all zero, since then we have expressed $ \vec{w}_{r+1} $ as a linear combination of $ \vec{w}_1,\cdots,\vec{w}_r $, contradict out assumption. Without loss of generality, assume that $ c_{r+1} $ is not zero. Repeat the same process when we solve for $ \vec{v}_1 $ to solve for $ \vec{v}_{r+1} $, we then find that $ \vec{v}_{r+1} $ is in the subspace generated by $ \vec{w}_1,\cdots,\vec{w}_{r+1},\vec{v}_{r+2},\cdots,\vec{v}_{m} $. By the induction assumption, these vectors also generate $ V $. Therefore vectors $ \vec{w}_1,\cdots,\vec{w}_m $ generate $ V $, and for $ n>m $ we can expressed $ \vec{w}_n $ as a linear combination of $ \vec{w}_1,\cdots,\vec{w}_m $. The theorem is then proved.
\end{proof}	
\begin{Corollary}
	Any two basis of the same vector space have the same number of elements.
\end{Corollary}
\begin{Definition}
	The number of vectors in a basis is called the \textit{dimension} of this vector space.
\end{Definition}
\begin{Definition}
	The \textit{maximal set of linearly independent elements} of a vector space $ V $ is a collection of linearly independent vectors such that adding extra "outside" vectors would result in the loss of its linearly independence.
\end{Definition}
\begin{Theorem}
	The maximal set of linearly independent elements of a vector space $ V $ is a basis for it.
\end{Theorem}
\begin{proof}
	Let this maximal set be $ \{\vec{v}_1,\cdots,\vec{v}_n \} $ and $ \vec{w} $ be a element of the vector space $ V $. By hypotheses the union of the maximal set and $ \vec{w} $ is linearly dependent, then
	\begin{equation}
		x_0 \vec{w} + x_1 \vec{v}_1 + \cdots + x_n \vec{v}_n = 0 \nonumber
	\end{equation}
	has a nontrivial solution. By the linearly independence of $ \{\vec{v}_1,\cdots,\vec{v}_n \} $, $ x_0 $ is not zero. Then we can solve for $ \vec{w} $
	\begin{equation}
		\vec{w}=- \frac{x_1}{x_0}\vec{v}_1 - \cdots -\frac{x_n}{x_0}\vec{v}_n \nonumber
	\end{equation}
\end{proof}
\begin{Corollary}
	For a vector space of dimension $ n $, any collection of $ n $ linearly independent vectors is a basis for it.
\end{Corollary}
\begin{Corollary}
	Let $ V $ be a vector space and let $ W $ be a subspace for it. If $ \dim W = \dim V $, then $ V=W $.
\end{Corollary}
\begin{Corollary}
	Any linearly independent collections of a vector space can be made to a basis for it by adding some (or $ 0 $) vectors that are/is linearly independent to its elements.
\end{Corollary}
\begin{Theorem}
	The dimension of the subspace $ W $ that does not consist of the zero vector alone of a vector space $ V $ is no greater than $ \dim V $.
\end{Theorem}
	
\section{Sums and Direct Sums}
\begin{Definition}
	If $ U $ and $ W $ are subspaces of a vector space $ V $, then $ U+W $, the set of all elements $ \vec{u}+\vec{w} $ with $ \vec{u} \in U $ and $ \vec{w} \in W $, is a subspace of $ V $, said to be generated by $ U $ and $ W $, and called the \textit{sum} of $ U $ and $ W $. $ V $ is a \textit{direct sum} of $ U $ and $ W $ if for any $ \vec{v} \in V $ there exist \textbf{unique} elements $ \vec{u}\in U $ an $ \vec{w}\in W $ such that $ \vec{v}=\vec{u}+\vec{w} $. When $ V $ is the direct sum of subspaces $ U $, $ W $, we write 
	\begin{equation}
		V = U \oplus W \nonumber
	\end{equation}
\end{Definition}
\begin{Theorem}
	Let $ V $ be a vector space over the field $ K $, and let $ U $, $ W $ be subspaces. If $ U+W = V $ and $ U\cap W = \{\vec{0} \}$, then $ V = U \oplus W $.
\end{Theorem}
\begin{Theorem}
	Let $ V $ be a finite dimensional vector space over the field $ K $. Let $ W $ be a subspace. Then there exists a subspace $ U $ such that $ V = W \oplus U $.
\end{Theorem}
\begin{Theorem}
	If $ V $ is a finite dimensional vector space and is the direct sum of subspaces $ U $ and $ W $, then
	\begin{equation}
		\dim V = \dim W + \dim U \nonumber
	\end{equation}
\end{Theorem}
\begin{Definition}
	The \textit{direct product} of two vector spaces $ U $ and $ W $ over the field $ K $ is the cartesian product of elements in $ U $ and $ W $, i.e $ (u_1,w_1)\in U \times W $. The vector addition and scalar multiplication are defined to be componentwise.
\end{Definition}
Immediately, the direct product of two vector spaces over the same field is a vector space.
\begin{Theorem}
	\begin{equation}
...
\end{Theorem}
\begin{proof}
	For two basis in each space $ \{\vec{u}_1,\cdots,\vec{u}_n \} $ and $ \{\vec{w}_1,\cdots,\vec{w}_m \} $, the collection of vectors in $ U \times W $ consists of $ \{(\vec{u}_1,\vec{0}),\cdots,(\vec{u}_n,\vec{0}),(\vec{0},\vec{w}_1),\cdots,(\vec{0},\vec{w}_m) \} $ is a basis for $ U \times W $ of $ n+m $ elements.
\end{proof}
The notion of direct sum and direct product can be extended to several factors with $ \sum $ and $ \prod $. In this circumstance vector addition and scalar multiplication are also defined to be componentwise.
...
\end{Definition}
\begin{Definition}
	A matrix is said to be \textit{symmetric} if it is equal to its transpose.
\end{Definition}
\begin{Definition}
	The \textit{unit} $ n \times n $ matrix, denoted by $ \matr{I}_n $, is the square matrix having all its components equal to $ 0 $ except the diagonal components, equal to $ 1 $.
\end{Definition}
\begin{Theorem}
...
\begin{Definition}
	Let $ V,W $ be vector spaces over $ K $, and let $ F:V \to W $ be a linear map. The \textit{kennel} of $ F $, denoted $ \Ker F $, is the set of elements $ \vec{v} \in V $ such that $ F(\vec{v}=\vec{0}) $.
\end{Definition}	
The kernel of a linear map on vector space is a subspace of the vector space.
\begin{Proposition}
	The following two conditions are equivalent
	\begin{enumerate}
		\item The kernel of $ F $ is $ \{\vec{0} \} $.
		\item $ F $ is injective.
	\end{enumerate}
\end{Proposition}
\begin{proof}
...
\end{proof}
\begin{Theorem}
	Let $ F:V \to W $ be a linear map whose kernel is $ \{\vec{0} \} $. If $ \vec{v}_1,\cdots,\vec{v}_n $ are linearly independent elements of $ V $, then $ F(\vec{v}_1),\cdots,F(\vec{v}_n) $ are linearly independent elements of $ W $.
\end{Theorem}
\begin{proof}
	content...
\end{proof}
\begin{Theorem}
	Let $ V $ be a vector space. Let $ L:V \to W $ be a linear map of $ V $ into another space $ W $. Then
	\begin{equation}
...
\end{proof}
\begin{Theorem}
	Let $ L:V \to W $ be a linear map. Assume that
	\begin{equation}
		\dim V = \dim W \nonumber
	\end{equation}
	If $ \Ker L = \{\vec{0} \} $, or if $ \Im L = W $, then $ L $ is bijective.
\end{Theorem}
\begin{proof}
...
\end{Theorem}
\begin{proof}
	Since $ p>n $, there are more variables than equations, and therefore nontrivial solutions exist.
\end{proof}
\begin{Theorem}
...
\end{Theorem}
\begin{proof}
	The nonpivot columns correspond to the free variables in $ \matr{Ax}=\vec{0} $, and since the number of pivot columns plus the number of nonpivot columns are the number of columns in the matrix, the proof completes.
\end{proof}
...
The theory of eigenvalues and eigenvectors developed for $ \Real^n $ applies equally well on $ \Complex^n $.
\begin{Theorem}
	Let $ \matr{A} $ be a real $ 2 \times 2 $ matrix with a complex eigenvalue $ \lambda=a-b \iu  (b \neq 0)$ and associated eigenvector $ \vec{v}\in \Complex^2 $. Then
	\begin{equation}
		\matr{A}=\matr{PC}\matr{P}^{-1},\quad \text{where}\quad \matr{P}=\begin{bmatrix}
		\Re \vec{v} & \Im \vec{v}
		\end{bmatrix} \quad \text{and}\quad \vec{C}=\begin{bmatrix}
		a &-b \\ 
		b &a
		\end{bmatrix} \nonumber
	\end{equation}
...
\begin{Definition}
	If a vector $ \vec{z} $ is orthogonal to every vector in a subspace $ W $ of $ \Real^n $, then $ \vec{z} $ is said to be \textit{orthogonal} to $ W $. The set of all vectors $ \vec{z} $ that are orthogonal to $ W $ is called the \textit{orthogonal complement} of $ W $ and denoted by $ W^\perp $.
\end{Definition}
\begin{Theorem}
	\begin{enumerate}
		\item A vector $ \vec{x} $ is in $ W^\perp $ iff $ \vec{x} $ is orthogonal to every vector in a set that spans $ W $.
		\item $ W^\perp $ is a subspace of $ \Real^n $.
	\end{enumerate}
\end{Theorem}
\begin{proof}
	\exercise
\end{proof}
\begin{Theorem}
	Let $ \matr{A} $ be an $ m \times n $ matrix. Then the orthogonal complement of the row space of $ \matr{A} $ is the nullspace of $ \matr{A} $, and the orthogonal complement of the column space of $ \matr{A} $ is the nullspace of $ \matr{A}^T $:
	\begin{equation}
		(\row \matr{A})^\perp = \nul \matr{A},\qquad (\col \matr{A})^\perp = \nul \matr{A}^T \nonumber
	\end{equation}
\end{Theorem}
\section{Orthogonal Sets}
\begin{Definition}
	A set of vectors in $ \Real^n $ is said to be an \textit{orthogonal set} if each pair of distinct vectors from the set is orthogonal.
\end{Definition}
\begin{Theorem}
	If $ S=\{\vec{u}_1,\cdots,\vec{u}_p \} $ is an orthogonal set of nonzero vectors in $ \Real^n $, then $ S $ is linearly independent and hence is a basis for the subspace spanned by $ S $.
\end{Theorem}
\begin{Definition}
	An \textit{orthogonal basis} for a subspace $ W $ of $ \Real^n $ is a basis for $ W $ that is also an orthogonal set.
\end{Definition}
\begin{Theorem}
	Each vector in a subspace of $ \Real^n $ has a unique representation as a linear combination of its orthogonal basis.
\end{Theorem}
\begin{Definition}
	The \textit{orthogonal projection} of $\vec{v}$ on an arbitrary non-zero vector $\vec{b}$ can be written as:
	\begin{equation}
	\proj_{\vec{b}}\vec{v} = \frac{\vec{v} \cdot \vec{b}}{\norm{\vec{b}}^2}\vec{b}
	\end{equation}
	Moreover, we can see that $\vec{v} - proj_{\vec{b}}\vec{v}$ is the vector component of $\vec{v}$ orthogonal to $\vec{b}$. The projection $ proj_{\vec{b}}\vec{v} $ is determined by the subspace $ \spn \{\vec{b}\} $, and we may call it the \textit{orthogonal projection onto} $ \spn \{\vec{b}\} $.
\end{Definition}
\begin{Definition}
	A set is an \textit{orthonormal set} if it is an orthogonal set of unit vectors. It is also an \textit{orthonormal basis} for a subspace spanned by it.
\end{Definition}
\begin{Theorem}
	An $ m \times n $ matrix $ \matr{U} $ has orthonormal columns iff $ \matr{U}^T \matr{U}=\matr{I} $.
\end{Theorem}
\begin{Theorem}
	Let $ \matr{U} $ be an $ m \times n $ matrix with orthonormal columns, and let $ \vec{x},\vec{y} \in \Real^n $. Then
	\begin{enumerate}
		\item $ \norm{\matr{U}\vec{x}}=\norm{\vec{x}} $.
		\item $ \norm{\matr{U}\vec{x}} \cdot \norm{\matr{U}\vec{y}} =\vec{x} \cdot \vec{y}$, and it equals zero iff $ \vec{x}\cdot \vec{y}=0 $.
	\end{enumerate}
Equivalently, they say that the linear mapping $ \vec{x}\mapsto  \matr{U}\vec{x} $ preserves lengths and orthogonality.
\end{Theorem}
\begin{Definition}
	An \textit{orthogonal matrix} is a square invertible matrix $ \matr{U} $ such that $ \matr{U}^{-1}=\matr{U}^T $.
\end{Definition}
\section{Orthogonal Projections}
\begin{Theorem}
	Let $ W $ be a subspace of $ \Real^n $. Then each $ \vec{y}\in \Real^n $ can be written uniquely in the form
	\begin{equation}
		\vec{y}=\hat{\vec{y}}+\vec{z} \nonumber
	\end{equation}
	where $ \hat{\vec{y}} $ is in $ W $ and $ \vec{z} $ is in $ W^\perp $. In fact, if $ \{\vec{u}_1,\cdots,\vec{u}_p \} $ is any orthogonal basis of $ W $, then
	\begin{equation}
		\hat{\vec{y}}= \proj_{\vec{u}_1}\vec{y}+\cdots +\proj_{\vec{u}_p}\vec{y} \nonumber
	\end{equation}
	and $ \vec{z}=\vec{y}-\hat{\vec{y}} $.
\end{Theorem}
The vector $ \hat{\vec{y}} $ is called the \textit{orthogonal projection of $ \vec{y} $ onto $ W $} and written as $ \proj_{W}\vec{y} $.
\begin{Theorem}
	Let $ W $ be a subspace of $ \Real^n $, $ \vec{y}\in\Real^n $, and $ \hat{\vec{y}} $ the orthogonal projection of $ \vec{y} $ onto $ W $. Then $ \hat{\vec{y}} $ is the closest point in $ W $ to $ \vec{y} $, in the sense that
	\begin{equation}
		\norm{\vec{y}-\hat{\vec{y}}}<\norm{\vec{y}-\vec{v}} \nonumber
	\end{equation}
	for all $ \vec{v}\in W $ distinct from $ \hat{\vec{y}} $.
\end{Theorem}
\begin{Theorem}
	If $ \{\vec{u}_1,\cdots,\vec{u}_p \} $ is an orthonormal basis for a subspace $ W \in \Real^n $, then
	\begin{equation}
		\proj_{W}\vec{y}=(\vec{y}\cdot \vec{u}_1)\vec{u}_1 + \cdots +(\vec{y}\cdot \vec{u}_p)\vec{u}_p\nonumber
	\end{equation}
	If $ \matr{U} =\begin{bmatrix}
	\vec{u}_1 &\cdots &\vec{u}_p 
	\end{bmatrix}$, then
	\begin{equation}
		\proj_{W}\vec{y}=\matr{U}\matr{U}^T \vec{y}\nonumber
	\end{equation}
\end{Theorem}
\chapter{Symmetric Matrices and Quadratic Forms}
\begin{Definition}
	A \textit{symmetric matrix} is a matrix such that it equals to the transpose of itself.
\end{Definition}
\begin{Definition}
	A \textit{quadratic form} on $ \Real^n $ is a function $ Q $ defined on $ \Real^n $ whose value at a vector $ \vec{x}\in \Real^n $ can be computed by an expression of the form $ Q(\vec{x})=\vec{x}\matr{A}^T \vec{x} $, where $ \matr{A} $ is an $ n \times n $ symmetric matrix. The matrix $ \matr{A} $ is called the \textit{matrix of the quadratic form}. The simplest example of a nonzero quadratic form is where the matrix of the quadratic form is the $ n \times n $ identity matrix.
\end{Definition}
\begin{Definition}
	A quadratic form $ Q $ is:
	\begin{enumerate}
		\item \textit{positive definite} if $ Q(\vec{x}) >0$ for all $ \vec{x}\neq \vec{0} $,
		\item \textit{negative definite} if $ Q(\vec{x}) <0$ for all $ \vec{x}\neq \vec{0} $, 
		\item \textit{indefinite} if $ Q(\vec{x}) $ assumes both positive and negative values.
	\end{enumerate}
\end{Definition}
\begin{Theorem}
	Let $ \matr{A} $ be an $ n \times n $ symmetric matrix. Then a quadratic form $ \vec{x}\matr{A}^T \vec{x}  $ is:
		\begin{enumerate}
		\item positive definite iff the eigenvalues of $ \matr{A} $ are all positive,
		\item negative definite iff the eigenvalues of $ \matr{A} $ are all negative, 
		\item indefinite iff the eigenvalues of $ \matr{A} $ has both positive and negative eigenvalues.
	\end{enumerate}
\end{Theorem}
\begin{Definition}
	A \textit{positive definite matrix} $ \matr{A} $ is a symmetric matrix for which the quadratic form is positive definite. The matrix is a \textit{positive semidefinite matrix} if its quadratic form is nonnegative. Other terms are defined analogously.
\end{Definition}
