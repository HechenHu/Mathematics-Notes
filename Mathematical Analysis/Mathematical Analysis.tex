...
\maketitle
\chapter{Preface}
	This note was took during my study of Mathematical Analysis using the book Zorich's \textit{Mathematical Analysis, Second Edition}. Some proof might just be summarized, and I'll make page references to the original proof in the book. The first 8 chapter come from the first book, the rest come from the second book.All page references goes to the corresponding book.
...
\begin{Definition}(Algebraic Numbers and Transcendental Numbers)
	...
	\begin{equation}
		a_0x^n + \cdot \cdot \cdot + a_{n-1}x + a_n = 0 \nonumber
	\end{equation}
...
\begin{Theorem}
	The inequality
	\begin{equation}
	|x_1+ \cdot \cdot \cdot x_n| \leqslant |x_1| + \cdot \cdot \cdot |x_n| \nonumber
	\end{equation}
...
\begin{Lemma}[Cauchy-Cantor]
	For any nested sequence $ I_1 \supset I_2 \supset \cdot \cdot \cdot \supset I_n \supset \cdot \cdot \cdot $ of closed intervals, there exists a point $ c\in \Real $ belonging to all of these intervals. \newpara
...
\begin{proof}
	...
	An algebraic equation of degree $ k $ with rational coefficient can be writtne in the reduced form $ x^k + r_1x^{k-1}+ \cdot \cdot \cdot + r_k = 0 $. Thus there are as many different algebraic equations of degree $ k $ as there are different ordered sets $ (r_1,...,r_k) $ of rational numbers, that is, a countable set. \newpara
...
\subsubsection{Passage to the Limit and the Arithmetic Operations}[p82-p84]
\begin{Definition}
...
\end{Statement}
\begin{proof}
	Let $ y_n = (1+\frac{1}{n})^{n+1} $ and $ n \geqslant 2 $. Use Bernoulli's inequality, we find that $ \frac{y_{n-1}}{y_n}>1 $, and since all terms are positive, this sequence is monotonically decreasing and hence, has a limit. Then 
	\begin{equation}
...
\begin{Corollary}
	The Bolzano-Weierstrass Lemma in its restricted and wider formulations follows from the Proposition we just proved.
\end{Corollary}
\begin{proof}
	content...
\end{proof}
